...
void ExportXML::openImport(bool allData)
{
    ...
    QByteArray text = ("ВЫБРАТЬ "
                       ...
                       "УПОРЯДОЧИТЬ ПО "
                       "FIO");
    queryTextFizLiz = (text);

    text = ("ВЫБРАТЬ "
            "ПаспортныеДанныеФизЛицСрезПоследних.ДокументСерия КАК PassportSeriya, "
            "ПаспортныеДанныеФизЛицСрезПоследних.ДокументНомер КАК PassportNomer, "
            "ПаспортныеДанныеФизЛицСрезПоследних.ДокументДатаВыдачи КАК PassportDataVidachi, "
            "ПаспортныеДанныеФизЛицСрезПоследних.ДокументКемВыдан КАК KemVidan "
            "ИЗ "
            "РегистрСведений.ПаспортныеДанныеФизЛиц.СрезПоследних КАК ПаспортныеДанныеФизЛицСрезПоследних "
            "ГДЕ "
            "ПаспортныеДанныеФизЛицСрезПоследних.ФизЛицо.Код = \"%1\"");
    queryTextPassport = (text);
    ...
    text = ("ВЫБРАТЬ ПЕРВЫЕ 1 "
            ...
            "УПОРЯДОЧИТЬ ПО "
            "Data УБЫВ");
    queryTextMedosmotr = (text);


    text = ("ВЫБРАТЬ ПЕРВЫЕ 1 "
            "СИТ_АРМ_ПротоколПроверкиЗнанийПромышленнойБезопасности.ПрограммаОбучения.Наименование КАК ProgrammaObucheniya, "
            ...
            "ИЗ "
            "Документ.СИТ_АРМ_ПротоколПроверкиЗнанийПромышленнойБезопасности КАК СИТ_АРМ_ПротоколПроверкиЗнанийПромышленнойБезопасности "
            "ГДЕ "
            "СИТ_АРМ_ПротоколПроверкиЗнанийПромышленнойБезопасности.Работники.Сотрудник.Код = \"%1\" "
            "УПОРЯДОЧИТЬ ПО "
            "Data УБЫВ");
    queryTextPromBezop = (text);
